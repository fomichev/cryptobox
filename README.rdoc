...
* {ExecJS}[https://github.com/sstephenson/execjs] - MIT
  Used to compile Handlebars templates to JS (will be made optional because
  users don't need to recompile templates)

* {EM-WebSocket}[https://github.com/igrigorik/em-websocket] - MIT
  Used in cbserve to serve encrypted JSON to Chrome plugin
...
* describe the following features
  ...
  - cryptobox.json
  - cbserve and how it talks to extension (and also chrome.embed configure option)
...
- Clean clipboard on exit only when used pasted something to it
- Add key binding to show popup (in overlay)
- Add login for unmatched sites (create new tab and login there) chrome.tabs.create({'url': chrome.extension.getURL('popup.html'), 'selected': 'true'});
