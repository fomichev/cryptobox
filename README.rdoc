...
When you're adding such form to the web forms storage (<tt>include/webform</tt>), you
...
form a tab in the HTML page. So, for example, if you have <tt>webform/google.com</tt>
and <tt>webform/yahoo.com</tt>, there will be a +webform+ page (or +Sites+
...
* {ExecJS}[https://github.com/sstephenson/execjs] - MIT
  ...
  We also install `therubyracer' GEM for cross platform Javascript engine (V8)
...
=== Ruby
* use one master command (+cb+) for actions like edit/create/etc (instead of tons of scripts)
...
* ? convert form bookmarklet to new cbserve when its ready

=== Desktop HTML
* update bootstrap and try to use http://twitter.github.com/bootstrap/examples/signin.html for login page
* add support for picking theme from http://bootswatch.com
* ? add some way to show hidden entries
...
* ? add possibility to connect to new version of cbserve
* Use the following bootstrap feature for hide/show functionality:
...
=== Read only server which serves encrypted data (evolution of cbserve)
* Will allow to add Firefox mobile extension
* We may need:
  - support some kind of certificates to prevent man-in-the-middle attacks (check secure WebSockets)
  - user will use different credentials to access this server (NOT MASTER PASS)
  - different users may publish their data to the same server (public)

* openssl req -new -x509 -days 365 -nodes -out cryptobox.crt -keyout cryptobox.key
  127.0.0.1 for common name
* Then navigate to https://127.0.0.1:22790/ in browser and add this certificate to exceptions list
* Commands format:
  enc_dec_key = sha(lowercase(username) + password)
  storage_key = sha(enc_dec_key + password)

  # get encrypted data
  { version: '1', user: '$username', key: '$storage_key', command: 'get' }
  # put encrypted data (override old)
  { version: '1', user: '$username', key: '$storage_key', command: 'put', data: {} }

  # try to register new user
  { version: '1', user: '$username', key: '$storage_key', command: 'register' }
  # delete existing user and all its data
  { version: '1', user: '$username', key: '$storage_key', command: 'unregister' }

  Ban for 1 second when wrong password or command is given.
* Add some option to config which tells to which server we have to push the data
