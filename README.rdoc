...
some encrypted file (<tt>private/cryptobox</tt>) where sensitive information is
stored. To edit it you use special executable (+cbedit+) which will open editor
...
a git clone.
So to install cryptobox, make git clone of this repository and add
...
because of the threading model (and who knows what other issues are).
You can get Ruby for Windows from http://www.ruby-lang.org; for Max OS X you
can use +brew+ to get Ruby; and on Linux system use the whatever package
manager is installed on your system to install it.

== Use

* Create database (creates empty <tt>private/cryptobox</tt>)
...
Whenever you edit cryptobox database, the backup file (<tt>private/backup/cryptobox</tt>)
is created with the previous version of your database. So if some unexpected
error happens, you can always restore your previous database contents.
...
* Chrome extension (planned) or browser bookmarklet to login to currently
  displayed page
...
To check the integrity of the database, Ruby code uses
...
* Embeds this encrypted data into pre-baked HTML page (look at <tt>templates/html/</tt> for
more details)
...
When you're adding such form to the logins storage (<tt>include/login</tt>), you
...
<tt>javascript:(function(){s=document.createElement('SCRIPT');s.type='text/javascript';s.src='https://<BOOKMARKLET_PATH>';document.getElementsByTagName('head')[0].appendChild(s);})();</tt>
...
colon and variable value on the right side of color without any quotes.
...
form a tab in the HTML page. So, for example, if you have <tt>login/google.com</tt>
and <tt>login/yahoo.com</tt>, there will be a +login+ page (or +Logins+
because of pre-defined translations) in the HTML document
...
<tt><%= @vars[:name] %></tt> inside the JSON file and will form particular
login/bookmark/etc. There is some special handling for the login entries,
where it's expected to have +form+ information with
...
For Chrome extension +visible+ attribute controls whether login will appear
in the unmatched section (so it will hide it from the list but will still
allow you to use hidden logins on the sites that you know).
...
form layout in the <tt>include/login</tt> directory (via provided bookmarklet)
...
reasons, you can always implement <tt>private/cryptobox</tt> parser yourself; the
format is YAML, so probably all languages have some support of it and +cbedit+
tool have useful <tt>--stdout</tt> argument which you can use to pipe your
plain text database to other tools safely.
...
look through the JSON and place <tt>&lg;%= @vars[:name] %&gt;</tt> and
<tt><%= @vars[:pass] %></tt> into appropriate
form fields (look for other logins JSON data in <tt>include/login</tt>, it will
become clear from the example what to do). Better yet, you can fill in the
form in the browser with the mentioned values and run the bookmarklet;
...
<tt>
# this is a comment
login/dropbox.com s@fomichev.me:
  pass: who knows

login/gmail.com s@fomichev.me:
  pass: really don''t know

login/gmail.com 2s@fomichev.me:
...
include:
  'login/xyz.com': { here goes JSON }
  'login/qwe.com': { here goes another JSON }
...
* {EM-WebSocket}[https://github.com/igrigorik/em-websocket] - MIT
  Used in cbserve to serve encrypted JSON to Chrome extension

  gem install em-websocket

==== The following components are required only for developers
...
* {Iconic}[https://github.com/somerandomdude/Iconic] - CC BY-SA 3.0
	...
	templates/chrome/icon.png

=== JavaScript
Some minimized version are obtained via http://closure-compiler.appspot.com/home

* {CryptoJS 3.0.2}[https://code.google.com/p/crypto-js/] - New BSD License

	templates/extern/CryptoJS

* {Random Seed 2.0}[http://davidbau.com/archives/2010/01/30/random_seeds_coded_hints_and_quintillions.html] - BSD

	templates/extern/seedrandom

* {jQuery 1.7.2}[http://jquery.com] - MIT

	templates/extern/jquery

* {Twitter Bootstrap 2.1.1}[https://github.com/twitter/bootstrap] - Apache License, Version 2.0

	templates/extern/bootstrap

* {Handlebars 1.0.rc.1}[https://github.com/wycats/handlebars.js] - MIT

	templates/extern/handlebars

* {jQuery Mobile 1.1.0}[http://jquerymobile.com] - MIT

	templates/extern/jquery-mobile

* {Clippy 7329b72360}[https://github.com/mojombo/clippy] - MIT

	templates/extern/clippy
...
With the following contents of +cryptobox+ file:
	...
	---
	pbkdf2_salt: FN4nebDeGcA=
	pbkdf2_iter: 2000
	aes_iv: K8llJdtkPBArV+LDzVd4/w==
	aes_keylen: 256
	hmac: NRqN9IbTNR/LM+eDfCkbOEEpz7Y=
	format_version: 5
	version: '0.7'
	timestamp: '2012-09-24T09:48:26+04:00'
	ciphertext: yP59as3mhtiQrzlr6rmWvn9RDF8Xgx+2hlcbHYD/6gIjWx0VvP0NwtCXxAgj4TqTzcz26BgtPdZHxw6Ydso5Tg==
...
Store your password in +pwd+ file and execute Perl script:
	...
	$ perl pbkdf2.pl 14de2779b0de19c0 $(grep pbkdf2_iter cryptobox | cut -d: -f2) < pwd > result

	$ cat result
	e623b705687b025e2a52d6b42aa4aab21ab469cce0914508f286be30be4e7d8d384df7e767dd431bc9689360e585ad4c
...
because each byte in hex encoding is represented by two characters)
	...
	$ cat secret
	e623b705687b025e2a52d6b42aa4aab21ab469cce0914508f286be30be4e7d8d
...
let's do it:
	...
	$ cat iv
	2bc96525db643c102b57e2c3cd5778ff

	$ grep ciphertext cryptobox | cut -d: -f2 | tr -d ' ' | tr -d "\n" > ciphertext
	$ cat ciphertext
	yP59as3mhtiQrzlr6rmWvn9RDF8Xgx+2hlcbHYD/6gIjWx0VvP0NwtCXxAgj4TqTzcz26BgtPdZHxw6Ydso5Tg==$

	$ openssl enc -base64 -A -d -aes-256-cbc -K $(cat secret) -iv $(cat iv) -in ciphertext -out plaintext
...
* describe the following features
  - broken logins (do they still work? recheck!)
  - cryptobox.json
  - cbserve and how it talks to extension
  - how to integrate include into yaml
* revise README.rdoc and add other missing information
* mention somewhere: supported fields for login entry are: user, pass, secret, note
* add comments to include/ JSON

=== Ruby
* create fifo somewhere in user's home directory with random name
...
* ? login -> webform
* fix race in cbedit pipe (between cbedit and reader)
* because we can't use pipe on windows, create plain file and use it for communication;
  afterwards write over with random data several times
  http://www.cs.auckland.ac.nz/~pgut001/pubs/secure_del.html
* set umask on unix when creating database
* try to use 10k iterations for pbkdf2
* generate new salt and iv on every +cbedit+ update and password change
* add --stdin flag to cbedit to support reading db contents from standard input
* ? use SHA1 instead of HMAC
...
=== Desktop HTML
* Fix black screen when auto locked
* Fix when number of entries is 0
* add tests support http://pivotal.github.com/jasmine/ and http://phantomjs.org/
* Check login with token
* ? Use icons http://fortawesome.github.com/Font-Awesome/
* !!! Share as many code as possible with chrome plugin

=== Chrome extension
- Add key binding to show popup (in overlay)
- ? Possibly add feature to plugin to create new login entries in the database (by just appending to plaintext in cbserve)
  This will require making cbserve more complex (we need to establish secure communication between cbserve and plugin to pass passwords around)

=== Firefox extension
- ? Try to support firefox also (seems to be possible to share huge amount of codebase)
