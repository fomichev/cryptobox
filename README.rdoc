...
=== Ruby
* On Ruby side only standard bindings to OpenSSL are used to manage database

* ExecJS - to compile Handlebars templates to JS (will be made optional because
  users don't need to recompile templates)
...
TODO: update! This is out of date and should be adapted to new database format!
...
=== Ruby
* make Handlebars templates compilation optional (because of ExecJS dependency)
...
* look for possible ways to integrate private/include into database itself (using --- YAML separators)

=== Javascript
* FIX AND CHECK: bookmarklets
...
=== Javascript Chrome plugin which talks to small Ruby server to get decrypted data (it tells it password, server decrypts data and passes JSON back)
- Add key binding to show popup http://developer.chrome.com/extensions/experimental.keybinding.html
- Add login for unmatched sites (create new tab and login there)
...
- Implement cbserve and add its support to extension (https://github.com/igrigorik/em-websocket) http://www.igvita.com/2009/12/22/ruby-websockets-tcp-for-the-browser/
- ? Also possibly add feature to plugin to create new login entries in the database (by just appending to plaintext in cbserve)
