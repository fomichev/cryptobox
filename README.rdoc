...
file (<tt>cryptobox/cryptobox.yaml</tt>) where the sensitive information is
...
* Create new database (creates empty <tt>cryptobox/cryptobox.yaml</tt>):
...
* Edit your database (it will also update html page at
  ...
  <tt>cryptobox/html/m.cryptobox.html</tt>):
...
* Embeds JavaScript and CSS (along with images) into HTML page and stores it
  under <tt>cryptobox/html</tt> directory (cryptobox.html - is a desktop
...
== Database example
Database example is located in `sample/cryptobox.yml` file.
...
=== Bookmarklets
* TEST FORM BOOKMARKLETS (may not work because of the latest changes)
