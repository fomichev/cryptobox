...
should set token field value to <tt>__token__</tt>; that will lead to a pop-up
dialog box on login asking you to provide the form data (you may have multiple
tokens within forms).
...
because of predefined translations) in the HTML document
...
form fields (look for the other logins' JSON data in <tt>include/login</tt>,
...
== Database example
	...
	include:
	  login/dropbox.com: { "name": "Dropbox 123", "address": "https://www.dropbox.com/", "form": { "action":"https://www.dropbox.com/login", "method": "post", "fields": { "login_email": "OOO <%= @vars[:name] %>", "login_password": "ZZZ <%= @vars[:pass] %>", "remember_me": "on", "cont": "/home" } } }
...
* describe the following features
  ...
  - in vim to read database :r !cbedit --stdout (enter password and press enter)
           to save database :w !cbedit --stdin
  - cbcat
...
=== Tests
* Apparently tests remove `private/' directory, find where
* Cucumber/Aruba tests
* ? Add unit tests

== GLOBAL
* rework JSON output {"login" => [/* logins */], "app" => [/* apps */], etc }
...
* because we can't use pipe on windows, create plain file and use it for communication;
  ...
  Also, add --no-pipe cbedit argument to be able to fallback to plain file as
  exchange method (also add smart wiping of this file)
...
* cbcreate: set umask on unix when creating database
* try to use 10k iterations for pbkdf2
* don't use minimized version, add normal ones (for debugging); we anyway have
  YUI step which minimizes them
* ? .html -> .rhtml
* ? login -> webform
...
* Fix black screen (from dialog) when auto locked
* Fix when number of entries is 0
* add spinner while calculating pbkdf2 (or at least change the title of the button color?)
* when click on site address in `details', open new window (don't go in current window)
* decouple bootstrap CSS settings from HTML (create additional meaningful containers)
