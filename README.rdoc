...
* fill bookmarklet: add support for 'cryptobox depot'
...
=== Read only server which serves encrypted data
* Rename to 'cryptobox depot'
* Add command to do registration and password change:
  - cryptobox depot register
  - cryptobox depot passwd
  - by default execute: cryptobox depot start (to start local server)
  - cryptobox depot certificate (to generate self signed certificates)
...
* Allow path.remote to specify remote server to use (in the form of
  [cryptobox://]<username>@<hostname>[:port]
  this way we will push cryptobox.json also to this server upon changes
* Probably use https://github.com/imanel/websocket-ruby for client and
  server

* openssl req -new -x509 -days 365 -nodes -out cryptobox.crt -keyout
  cryptobox.key
  127.0.0.1 for common name
* Then navigate to https://127.0.0.1:22790/ in browser and add this
  certificate to exceptions list
* Commands format:
