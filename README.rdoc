...
* Embeds this encrypted data into pre-baked HTML page (look at
  <tt>templates/html/</tt> for more details);
...
* {EM-WebSocket}[https://github.com/igrigorik/em-websocket] - MIT
  Used to serve encrypted JSON to Chrome extension
...
* describe the following features
  ...
  - cryptobox.json
  - 'cryptobox serve' and how it talks to extension
...
=== Ruby
* add config options to disable building of embedded/depot versions
...
=== Bookmarklets
* TEST FROM BOOKMARKLET! (may not work because of the latest changes)
...
* ? convert form bookmarklet to new 'cryptobox serve' when its ready

=== Common JS
* eliminate cryptobox.ui.init (integrate in cryptobox.open and convert chrome
  plugin to its format)

=== Desktop HTML
* build version which can connect to 'cryptobox depot'
* ? add some way to show hidden entries
* Check login with token
* ? add possibility to connect to new version of 'cryptobox serve'
...
=== Chrome extension
* Align locked page with desktop version
...
=== Read only server which serves encrypted data (evolution of 'cryptobox serve')
* Rename to 'cryptobox depot'
...
* Commands format:
  aes_key = pbkdf2(salt, password, iter)
  user_key = sha256(hostname + username)

  # register new user
  # # send registration request
  depot <- { command: 'register', data: '<user_key>', error: 0 }
  # # server generates random 512bit data and asks user to sing it
  depot -> { command: 'sign', data: '0011223344....', error: 0 }
  # # user received data, does sha256(aes(data, aes_key)) and sends result back
  depot <- { command: 'sign_ack', data: 'aabbccddee....', error: 0 }
  # # server return ok (and stores returned key) or fail (user already exist)
  depot -> { command: 'register_ack', data: {}, error: 0/1 }


  # authorization
  # # send authorization request
  depot <- { command: 'hello', data: '<user_key>', error: 0 }
  # # server takes stored random 512bit data and sends it to the user
  depot -> { command: 'hello_ack', data: '0011223344....', error: 0 }


  # # user received data, does token = sha256(aes(data, aes_key)) and uses
  # # it for the next commands


  # get encrypted data
  depot <- { command: 'get', token: '<token>' }
  # put encrypted data (override old)
  depot <- { command: 'put', token: '<token>', data: {} }

  # delete existing user and all its data
  depot <- { command: 'unregister', token: '<token>' }

  Ban for 1 second when wrong password or command is given.

  # each message should also contain cryptobox_version and format fields

* Add some option to config which tells to which server we have to push the data
  or store this information directly in the cryptobox itself
