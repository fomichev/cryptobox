...
=== Ruby
* On Ruby only standard bindings to OpenSSL are used to manage database
...
* Bootstrap desktop
  ...
  - Rework login.js (remove stupid string conversions)
  - Add filter
  - Set empty tag instead of __default__
  - Probably try to use another layout
  - Share as many code as possible with chrome plugin
  - move template/html/* -> template/
...
* Chrome plugin which talks to small Ruby server to get decrypted data (it tells it password, server decrypts data and passes JSON back)
  ...
  - Add filter
  - Add passwords generator (with possible diceware support; and use random.org when possible to generate passwords)
