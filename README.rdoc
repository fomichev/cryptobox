...
have `form` information with `<%= @vars[:name] %>` and `<%= @vars[:pass] %>` variables.
...
look through the JSON and place `<%= @vars[:name] %>` and `<%= @vars[:pass] %>` into appropriate
...
form in the browser with `<%= @vars[:name] %>` and `<%= @vars[:pass] %>` and run the bookmarklet;
